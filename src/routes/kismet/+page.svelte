<script lang="ts">
  ...
  async function startKismet() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const startupInterval = setInterval(() => {
          (async () => {
            ...
            try {
              await fetch(`http://${window.location.hostname}:2501/system/status.json`, {
                mode: 'no-cors'
              });
      ...
      kismetStatus = 'stopped';
      hasError = true;
      errorMessage = error instanceof Error ? error.message : 'Failed to start Kismet service';
    }
  }
  ...
  <header class="bg-gray-900 border-b border-gray-800 p-4">
    <div class="container mx-auto flex items-center justify-between">
      <div class="flex items-center gap-4">
        ...
        <a href="/tactical-map-simple" class="ml-4 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded transition-colors flex items-center gap-2">
      ...
      <button 
        ...
        class="px-6 py-2 rounded font-medium transition-all duration-200 flex items-center gap-2
          ...
          {kismetStatus === 'starting' || kismetStatus === 'stopping' ? 'bg-gray-600 text-gray-300 cursor-not-allowed' : ''}"
      >
      ...
      <div class="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-gray-900 via-black to-gray-800">
        <div class="text-center max-w-2xl mx-auto p-8">
          ...
          <div class="relative mb-8">
            <svg class="w-32 h-32 text-cyan-500 mx-auto opacity-80" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12 mb-12">
              <div class="bg-gray-800/50 rounded-lg p-6 border border-gray-700">
                <svg class="w-8 h-8 text-cyan-400 mb-3" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                </svg>
                <h3 class="text-white font-semibold mb-2">Real-time Detection</h3>
                <p class="text-gray-400 text-sm">Monitor wireless networks and devices in real-time</p>
              </div>
              ...
              <div class="bg-gray-800/50 rounded-lg p-6 border border-gray-700">
                ...
                <p class="text-gray-400 text-sm">Analyze signal strength and device behavior</p>
              </div>
              
              <div class="bg-gray-800/50 rounded-lg p-6 border border-gray-700">
                <svg class="w-8 h-8 text-cyan-400 mb-3" fill="currentColor" viewBox="0 0 20 20">
                  <path fill-rule="evenodd" d="M12 1.586l-4 4v12.828l4-4V1.586zM3.707 3.293A1 1 0 002 4v10a1 1 0 00.293.707L6 18.414V5.586L3.707 3.293zM17.707 5.293L14 1.586v12.828l3.707 3.707A1 1 0 0018 17.414V6a1 1 0 00-.293-.707z"/>
                </svg>
                <h3 class="text-white font-semibold mb-2">Tactical Mapping</h3>
                <p class="text-gray-400 text-sm">Visualize device locations on interactive maps</p>
              </div>
            </div>
