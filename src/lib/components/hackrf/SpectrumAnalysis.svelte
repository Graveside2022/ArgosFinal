...
<div class="glass-panel rounded-xl p-6">
	...
	<div class="grid grid-cols-1 md:grid-cols-4 gap-4">
		<div class="glass-panel-light rounded-lg p-4">
			<span class="font-mono text-xs text-text-muted uppercase tracking-wider">Peak Power</span>
			<p class="font-mono text-h5 text-neon-green mt-1">
				{$spectrumData?.peak_power ? `${$spectrumData.peak_power.toFixed(1)} dBm` : '--'}
			</p>
		</div>
		<div class="glass-panel-light rounded-lg p-4">
			<span class="font-mono text-xs text-text-muted uppercase tracking-wider">Peak Frequency</span>
			<p class="font-mono text-h5 text-accent-primary mt-1">
				{$spectrumData?.peak_freq ? `${$spectrumData.peak_freq.toFixed(3)} MHz` : '--'}
			</p>
		</div>
		<div class="glass-panel-light rounded-lg p-4">
			<span class="font-mono text-xs text-text-muted uppercase tracking-wider">Average Power</span>
	...
	{#if $signalHistory.length > 0}
		<div class="mt-6">
			<h4 class="font-mono text-xs text-text-muted uppercase tracking-wider mb-3">Recent Signals</h4>
			<div class="space-y-2 max-h-40 overflow-y-auto custom-scrollbar">
				{#each $signalHistory.slice(-5).reverse() as signal}
					<div class="glass-panel-light rounded-lg p-3 flex items-center justify-between text-caption">
						<span class="font-mono text-text-secondary">{new Date(signal.timestamp).toLocaleTimeString()}</span>
						<span class="font-mono text-accent-primary">{signal.frequency.toFixed(3)} MHz</span>
						<span class="font-mono {signal.power > -50 ? 'text-neon-green' : 'text-text-muted'}">{signal.power.toFixed(1)} dBm</span>
					</div>
				{/each}
			</div>
		</div>
