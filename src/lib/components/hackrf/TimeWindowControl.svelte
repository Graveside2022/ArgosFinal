<script lang="ts">
	import { timeWindowFilter, formatAge as _formatAge, getAgeColor as _getAgeColor, type TimeWindowConfig } from '$lib/services/hackrf/timeWindowFilter';
	import { onMount, onDestroy } from 'svelte';
	
	// Get stores from the filter
	const { signals: _signals, activeSignals: _activeSignals, fadingSignals: _fadingSignals, stats } = timeWindowFilter;
	...
	$: selectedPresetData = selectedPreset && presets[selectedPreset as keyof typeof presets] ? presets[selectedPreset as keyof typeof presets] : null;
	...
	// Auto-remove timer
	let autoRemoveInterval: number | null = null;
...
<div class="glass-panel rounded-xl p-6 space-y-6">
	...
	<div class="flex items-center justify-between">
		<h3 class="text-h5 font-heading font-semibold text-text-primary flex items-center">
			<svg class="w-5 h-5 mr-2 text-neon-cyan" fill="currentColor" viewBox="0 0 20 20">
				<path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415l-2.536-2.536V6z" clip-rule="evenodd"/>
			</svg>
			Time Window Filter
		</h3>
		<button 
		...
		>
			<svg class="w-5 h-5 transform transition-transform {showAdvanced ? 'rotate-180' : ''}" fill="currentColor" viewBox="0 0 20 20">
	...
	<div class="grid grid-cols-3 gap-3">
		...
		<div class="text-center">
			<div class="text-2xl font-mono font-bold text-text-secondary">{$stats.totalSignals}</div>
	...
	{#if showAdvanced}
		<div class="space-y-4 pt-4 border-t border-border-primary">
			<!-- Window Duration -->
			<div>
				<label for="windowDuration" class="flex items-center justify-between text-sm font-medium text-text-secondary mb-2">
			...
			<!-- Fade Start -->
			<div>
				<label for="fadeStart" class="flex items-center justify-between text-sm font-medium text-text-secondary mb-2">
			...
			<!-- Max Age -->
			<div>
				<label for="maxAge" class="flex items-center justify-between text-sm font-medium text-text-secondary mb-2">
