...
export class TestDataGenerator {
  ...
  generateBluetoothSignals(count: number, options: SignalOptions = {}): Signal[] {
    ...
    for (let i = 0; i < count; i++) {
      signals.push({
        ...
        metadata: {
          type: 'bluetooth',
          deviceClass: ['phone', 'headphones', 'watch', 'beacon'][Math.floor(Math.random() * 4)],
          bluetoothVersion: ['4.0', '4.2', '5.0', '5.1', '5.2'][Math.floor(Math.random() * 5)]
        }
      });
    }
  ...
  generateDroneSignals(count: number, options: DroneOptions = {}): DroneSignal[] {
    ...
    for (let d = 0; d < dronesCount; d++) {
      ...
      const startLon = -74.0060 + (Math.random() - 0.5) * 0.1;
      ...
      for (let i = 0; i < signalsPerDrone; i++) {
        ...
        signals.push({
          ...
          velocity: this.getDroneVelocity(pattern),
          metadata: {
            type: 'drone',
            droneId,
            pattern,
            manufacturer: ['DJI', 'Parrot', 'Autel', 'Skydio'][Math.floor(Math.random() * 4)],
  ...
  generateMixedSignals(totalCount: number, options: SignalOptions = {}): Signal[] {
    ...
    const wifiSignals = this.generateWiFiSignals(
      Math.floor(totalCount * distribution.wifi), options
    );
    const bluetoothSignals = this.generateBluetoothSignals(
      Math.floor(totalCount * distribution.bluetooth), options
    );
    const cellularSignals = this.generateCellularSignals(
      Math.floor(totalCount * distribution.cellular), options
    );
    const droneSignals = this.generateDroneSignals(
      Math.floor(totalCount * distribution.drone), options
    );
  ...
  private getAreaCenter(area?: string): { lat: number; lon: number } {
    const centers = {
      urban: { lat: 40.7128, lon: -74.0060 },      // NYC
