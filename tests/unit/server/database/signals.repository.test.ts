...
describe('SignalsRepository', () => {
  ...
  describe('findById', () => {
    it('should return a signal when found', () => {
      ...
      const mockStatement = { get: mockGet };
      vi.mocked(db.prepare).mockReturnValue(mockStatement as any);
    ...
    it('should handle database errors gracefully', () => {
      ...
      // Act & Assert
      expect(() => signalsRepository.findById('test-id')).toThrow('Database connection failed');
      expect(db.prepare).toHaveBeenCalledWith('SELECT * FROM signals WHERE signal_id = ?');
  ...
  describe('findRecent', () => {
    it('should return recent signals with default limit', () => {
      ...
      // Assert
      expect(db.prepare).toHaveBeenCalledWith('SELECT * FROM signals ORDER BY timestamp DESC LIMIT ?');
  ...
  describe('Edge cases and boundary conditions', () => {
    ...
    it('should handle undefined or null database instance', () => {
      ...
      // Restore
      vi.mocked(db).prepare = originalDb.prepare;
    });
  });
});