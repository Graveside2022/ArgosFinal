...
describe('Realistic Data Load Tests', () => {
  ...
  it('Urban environment - 1 hour', () => {
    ...
    for (let i = 0; i < signals.length; i += batchSize) {
      ...
      batch.forEach(signal => {
        dbService.insertSignal(signal as Parameters<typeof dbService.insertSignal>[0]);
      });
  ...
  it('Event scenario - 4 hours', async () => {
    ...
    for (let hour = 0; hour < 4; hour++) {
      ...
      const wifiHour = dataGenerator.generateWiFiSignals(
        Math.floor(50000 * loadFactor),
        {
          area: 'stadium',
          timeSpan: 3600000,
      ...
      const droneHour = dataGenerator.generateDroneSignals(
        Math.floor(500 * loadFactor),
        {
    ...
    // Performance assertions
    expect(report.avgResponseTime).toBeLessThan(2000); // < 2s avg response
    expect(report.peakMemoryUsage).toBeLessThan(1024 * 1024 * 1024); // < 1GB
    expect(report.errorRate).toBeLessThan(0.01); // < 1% errors
  ...
  it('24-hour continuous operation', async () => {
    ...
    for (let hour = 0; hour < 24; hour++) {
      ...
      for (let batch = 0; batch < batchCount; batch++) {
        const batchSignals = signals.slice(
          batch * batchSize,
          (batch + 1) * batchSize
        );
    ...
    console.warn('24-hour Operation Report:', {
      totalSignals,
      peakLoad,
      avgBatchTime: (report as { avgMetric: (metric: string) => number }).avgMetric('batch_time'),
      maxMemory: (report as { maxMetric: (metric: string) => number }).maxMetric('memory_usage') / (1024 * 1024), // MB
  ...
  for (let i = 0; i < actions; i++) {
    ...
    switch (action) {
      ...
      case 2: {
        ...
        const _centerLon = -74.0060 + (Math.random() - 0.5) * 0.1;
...
function generateDayProfile(): number[] {
  ...
  return [
    0.2, 0.15, 0.1, 0.1, 0.15, 0.3,    // 00:00 - 06:00 (night/early morning)
    0.5, 0.7, 0.9, 0.95, 0.9, 0.85,    // 06:00 - 12:00 (morning peak)
    0.8, 0.85, 0.9, 0.95, 1.0, 0.95,   // 12:00 - 18:00 (afternoon peak)
    0.9, 0.8, 0.7, 0.5, 0.4, 0.3        // 18:00 - 00:00 (evening decline)
  ];
}