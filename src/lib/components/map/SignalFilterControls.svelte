<script lang="ts">
  ...
  import { signalFilter, type FilteringOptions, DRONE_FREQUENCY_BANDS, INTERFERENCE_BANDS } from '$lib/services/map/signalFiltering';
  ...
  let enabledBands = new Map<string, boolean>([
    ...DRONE_FREQUENCY_BANDS.map(band => [band.name, true]),
    ...INTERFERENCE_BANDS.map(band => [band.name, false])
  ]);
  ...
  function applyFilters() {
    ...
    const enabledFreqBands = [...DRONE_FREQUENCY_BANDS, ...INTERFERENCE_BANDS]
      .filter(band => enabledBands.get(band.name));
    
    const excludedFreqBands = [...DRONE_FREQUENCY_BANDS, ...INTERFERENCE_BANDS]
      .filter(band => !enabledBands.get(band.name));
...
<div class="signal-filter-controls {showControls ? 'expanded' : 'collapsed'}">
  <div class="controls-header">
    ...
    >
      <svg class="w-4 h-4 transform transition-transform {showControls ? 'rotate-180' : ''}" 
  ...
  {#if showControls}
    <div class="controls-body">
      ...
      >
        ...
        <svg class="w-4 h-4 ml-1 transform transition-transform {showAdvanced ? 'rotate-180' : ''}" 
             fill="currentColor" viewBox="0 0 20 20">
          <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"/>
        </svg>
      </button>
                ...
                {#each DRONE_FREQUENCY_BANDS as band}
                  <label class="freq-toggle">
                    <input
                      ...
                      on:change={(e) => {
                        enabledBands.set(band.name, e.currentTarget.checked);
                        applyFilters();
                      }}
                      class="checkbox"
                    />
                    <span class="freq-label">
                      {band.name}
                      <span class="freq-range">({band.minFreq}-{band.maxFreq} MHz)</span>
                    </span>
                  </label>
                {/each}
              </div>
            ...
            <div class="grid grid-cols-2 gap-2">
              <div>
                ...
                <input
                  ...
                  on:change={(e) => {
                    filterOptions.timeWindow = parseInt(e.currentTarget.value) * 1000;
                    applyFilters();
                  }}
                  class="number-input"
                />
