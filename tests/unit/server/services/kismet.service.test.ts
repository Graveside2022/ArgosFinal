...
import type { KismetDevice, GPSPosition } from '$lib/server/services/kismet.service';
...
describe('KismetService', () => {
  ...
  describe('getDevices()', () => {
    ...
    it('should fallback to Method 2 when KismetProxy.getDevices fails', async () => {
      ...
      expect(KismetProxy.getDevices).toHaveBeenCalled();
      expect(KismetProxy.proxyGet).toHaveBeenCalledWith(expect.stringContaining('/devices/last-time/'));
      expect(result.source).toBe('kismet');
      expect(result.error).toBeNull();
      expect(result.devices).toHaveLength(1);
      expect(result.devices[0]).toMatchObject({
    ...
    it('should limit devices from summary endpoint to 50', async () => {
      ...
      const manyDevices = Array(100).fill(null).map((_, i) => ({
  ...
  describe('transformKismetDevices()', () => {
    ...
    it('should handle missing/null values with defaults', () => {
      ...
      expect(Math.abs(device.location.lat - mockGPSPosition.latitude)).toBeLessThanOrEqual(0.001); // Half of LOCATION_VARIANCE
      expect(Math.abs(device.location.lon - mockGPSPosition.longitude)).toBeLessThanOrEqual(0.001);
    });

    it('should calculate location with variance when location is missing', () => {
      const kismetDevices = Array(10).fill(null).map((_, i) => ({
      ...
      result.forEach(device => {
        expect(Math.abs(device.location.lat - mockGPSPosition.latitude)).toBeLessThanOrEqual(0.001);
        expect(Math.abs(device.location.lon - mockGPSPosition.longitude)).toBeLessThanOrEqual(0.001);
      });
